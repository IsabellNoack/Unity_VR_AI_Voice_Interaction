# Unity_VR_AI_Voice_Interaction

## Overview
This project is part of the first semester of my Master's program in Human-Computer Interaction. It serves as a testing environment for exploring various technologies:

- **OpenAI API**
- **Meta Voice SDK**
- Interactions with **Meta Quest 3**

## Requirements
To work with this project, you will need:

- Unity Editor (compatible version for the project).
- Meta Quest 3 headset.
- Access to OpenAI API credentials.
- Meta Voice SDK setup.

## Setup Instructions
1. Clone this repository to your local machine.
2. Open the project in Unity Editor (version 6000.3.11f1).
3. Ensure the required packages are installed.
4. Configure the OpenAI API credentials by setting an environment variable for your local user with variable name `OPENAI_API_KEY` and set its value to your OpenAI API key.
5. Connect your Meta Quest 3 headset for testing.